...
# serial application(s) and place them in your LAUNCHER_WORKDIR
...
export LAUNCHER_JOB_FILE={LAUNCHER_JOB_FILE}
export LAUNCHER_WORKDIR=$(pwd)
#
# Variable description:
#
#  EXECUTABLE     = full path to the job launcher executable
#  LAUNCHER_JOB_FILE   = text input file which specifies
...
if [ ! -f $LAUNCHER_WORKDIR/$LAUNCHER_JOB_FILE ]; then
	echo " "
	echo "Error: unable to find input control file $LAUNCHER_JOB_FILE."
	echo " "
	echo "Job not submitted."
	exit
fi
...
$TACC_LAUNCHER_DIR/paramrun $EXECUTABLE $LAUNCHER_JOB_FILE
...
def generate_lonestar_launcher(results, launcher_file):
    ...
    if results.job:
        launcher_file.write(parametric_job_submission.format(LAUNCHER_JOB_FILE=results.job))


def generate_stampede2_launcher(results, launcher_file):
    """
    ...
    if not results.num_nodes:
        # If a number of nodes is not explicitly specified, calculate based on
        # number of commands in jobs file, and optional wayness.
        if results.job:
        ...
        # A number of nodes has been specified. Default wayness is 16 on Stampede.
        # Use specified wayness to calculate total tasks.
        num_nodes = results.num_nodes
        num_nodes_line = '#SBATCH -N {}\n'.format(num_nodes)
    ...
    launcher_file.write(stampede2_parameters.format(
        name=results.name,
        num_jobs=num_jobs,
...
def check_time_and_queue(results):
    ...
    queues = {'ls4': ['normal', 'development', 'largemem', 'gpu', 'vis', 'serial'],
              'stampede2': ['normal', 'development', 'largemem', 'serial', 'large', 'request', 'normal-mic', 'gpu', 'gpudev', 'vis']}
    ...
    if hh == 0 and mm == 0 and ss == 0:
        sys.exit('Please enter a non-zero time value.')
    if mm > 59 or ss > 59:
        sys.exit(results.time + ' is not a valid time.')

    if host == "ls4" and results.queue == 'development':
        time_limit = 1
    elif host == "ls4" and results.queue == 'serial':
        time_limit = 12
    elif host == "stampede2" and results.queue == 'development':
        time_limit = 2
    elif host == "stampede2" and \
            results.queue in ['gpudev', 'visdev']:
        time_limit = 4
    elif host == "stampede2" and results.queue == 'vis':
        time_limit = 8
    elif host == "stampede2" and results.queue == 'serial':
        time_limit = 12
    elif host == "stampede2" and \
            results.queue in ['normal', 'largemem', 'normal-mic']:
        time_limit = 48
    elif host == "ls5" and results.queue == 'development':
        time_limit = 2
    elif host == "ls5" and results.queue == 'gpu':
        time_limit = 24
    elif host == "ls5" and results.queue == 'vis':
        time_limit = 8
    elif host == "ls5" and \
            results.queue in ['normal', 'largemem', 'hugemem']:
        time_limit = 48
    else:
        time_limit = 24
...
def main():
    # Get environment variables, os.environ.get returns 'none' if the
...
Report problems to rt-other@ccbb.utexas.edu''')
    ...
    required.add_argument('-n', action='store', dest='name',
                          required=True, help='The name of your job.')
    required.add_argument('-t', action='store', dest='time', required=True,
                          help='The time you want to give to your job. Format: hh:mm:ss')
    commands = parser.add_argument_group(
        'Commands', 'You must use at least one of these options to submit your commands for TACC.')
    commands.add_argument('-j', action='store', dest='job',
                          help='The name of the job file containing your commands.')
    commands.add_argument('-b', action='store', dest='bash_commands', default='',
                          help='A string of Bash commands that are executed before the parametric jobs are launched.')
    optional = parser.add_argument_group('Optional')
    optional.add_argument('-q', action='store', dest='queue', default='development',
                          help='The TACC allocation for job submission. Default="development"')
    optional.add_argument('-a', '-A', action='store', dest='allocation', nargs='?', default=env_allocation,
                          help='The TACC allocation for job submission. You can set a default ALLOCATION environment variable.')
    optional.add_argument('-m', action='store', dest='modules', default='',
                          help='A list of module commands. The "launcher" module is always automatically included. Example: -m "module swap intel gcc; module load bedtools"')
    optional.add_argument('-w', action='store', dest='wayness', type=int,
                          help='Wayness: the number of commands you want to give each node. Default= 12 on Lonestar, 16 on Stampede.')
    optional.add_argument('-N', action='store', dest='num_nodes', type=int,
                          help='Number of nodes to request. You probably don\'t need this option. You ONLY need it if you want to run a job list that isn\'t defined at the time you submit the launcher.')
    optional.add_argument('-e', action='store', dest='email', nargs='?', const=env_email_address,
                          help='Your email address if you want to receive an email from Lonestar when your job starts and ends. Without an argument, it will use a default EMAIL_ADDRESS environment variable.')
    optional.add_argument('-l', action='store', dest='launcher',
                          help='The name of the *.sge launcher script that will be created. Default="<name>.sge"')
    optional.add_argument('-s', dest='submit_cmd', action='store_true',
                          help='Echoes the launcher filename to stdout.')
    deprecated = parser.add_argument_group('Deprecated', 'Rewrite any scripts that use this.')
    deprecated.add_argument('-H', action='store', dest='hold',
                            help='Hold job in queue until the job specified in this option finishes.')
    ...
    if host == "ls4":
        ...
        generate_stampede2_launcher(results, launcher_file)
    elif host == "ls5":
